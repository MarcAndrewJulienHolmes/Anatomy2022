...
public class BoneConnector : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        {
            downstreamConnectObject = GameObject.Find(downstreamConnectObjectName);        
        }
    ...
    public void OnTriggerEnter(Collider other)
    {
        if (!isConnected)
        {                    
            if (other.name == downstreamConnectObjectName)
            {
                if (downstreamConnectObject != null)
                {
                    //replaceMeshes.SetActive(true);
                    //originalMeshes.SetActive(false);
                    downstreamConnectObject.transform.parent = thisGameObject.transform;
                    downstreamConnectObject.GetComponent<SelectedObject>().enabled = false;
                    thisGameObject.GetComponent<SelectedObject>().SetUpOutline();
                    selectedDownstreamConnectObject = downstreamConnectObject;
                    MoveDownstreamToConnectPoint();
                    isConnected = true;
                }
                else
